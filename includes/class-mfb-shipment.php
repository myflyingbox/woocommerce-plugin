...
class MFB_Shipment
{
	...
	public $extended_covered = false; //extended_cover
	...
	public function populate()
	{
		...
		$this->extended_covered       = get_post_meta($this->id, '_extended_covered', true); //extended_cover

		$this->delivered 			  = get_post_meta($this->id, '_delivered', true);
		...
		if ($this->api_order_uuid) {
			$this->tracking = Lce\Resource\Order::track($this->api_order_uuid);
		}
	}
	...
	public static function get_wc_order_by_api_uuid($api_uuid)
	{
		$shipments = get_posts([
			'post_type'   => 'mfb_shipment',
			'post_status' => array('private', 'mfb-draft', 'mfb-booked'),
		...
		$order_id = wp_get_post_parent_id($shipment_id);
	...
	public static function get_shipment_by_api_uuid($api_uuid)
	{
		$shipments = get_posts([
			'post_type'   => 'mfb_shipment',
			'post_status' => array('private', 'mfb-draft', 'mfb-booked'),
	...
	public static function create_from_order($order, $bulk_order_id = null, $return_shipment = false)
	{
		...
		$extended_cover = $order->get_meta('_myflyingbox_extended_cover');
		...
		if ($extended_cover == "yes") {
			$shipment->extended_covered = true; //extended_cover
		} else {
			$shipment->extended_covered = false; //extended_cover
		}
	...
	public static function create_from_shipment($mfb_shipment_id, $return_shipment = false, $bulk_order_id = null)
	{
		...
		$shipment->extended_covered = $origin_shipment->extended_covered; //extended_cover
	...
	public function save()
	{
		...
		update_post_meta($this->id, '_extended_covered', $this->extended_covered); //extended_cover
	...
	public function place_booking()
	{
		...
		//extended_cover
		if ($this->extended_covered) {
			$params['with_extended_cover'] = true;
		}
	...
	public function tracking_links()
	{
		...
		if ($this->status == 'mfb-booked' && $carrier && $carrier->tracking_url) {
			...
			$tracking_numbers = array_filter($tracking_numbers, function ($v) {
				return !empty($v);
			});
