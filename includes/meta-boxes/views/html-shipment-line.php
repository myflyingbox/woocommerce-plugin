				...
				<?php if ($shipment->quote) { ?>
					...
					<select name="_mfb_selected_offer" class="offer-selector" style="width: 250px; font-size: 0.9em;">
						...
						<?php foreach ($offers as $offer) {
							$show_price = $offer->formatted_price();
							// if($offer->is_extended_cover() && $extended_cover == "yes") {
							// 	$show_price = $offer->formatted_extended_cover_price();
							// }
							...
							echo ">" . $offer->full_service_name() . " - " . $show_price . "</option>";
						}
						?>
					</select>
					...
					if ($shipment->offer && $shipment->offer->is_extended_cover()) {
						$checked = "";
		...
		<div class="mfb-booked-offer">
			<div class="offer-details" data-offer_id="<?php echo $shipment->offer->id ?>">
				<p>
					<?php
					//extended_cover
					$offer_price = $shipment->offer->formatted_price();
					// if($shipment->offer->is_extended_cover() && $shipment->extended_covered) {
					// 	$offer_price = $shipment->offer->formatted_extended_cover_price();
					// }
					echo $shipment->offer->full_service_name() . ' (' . $offer_price . ')';
					if ($shipment->offer->is_extended_cover() && $shipment->extended_covered) {
						echo '<br/>';
			...
			<div class="last-status">
				<?php
				if (isset($shipment->tracking) && $shipment->tracking) {
						...
						if (!empty($tracking->events)) {
							...
							if ($latest_event) {
								...
								if (isset($latest_event->code) &&  $latest_event->code == "delivered") {
									update_post_meta($shipment->id, '_delivered', 1);
								} else {
									if (metadata_exists('post', $shipment->id, '_delivered')) {
										delete_post_meta($shipment->id, '_delivered');
									}
								}
							}
						}
			...
			<button type="button" class="button button-primary trackthis" data-offer_id="<?php echo $shipment->id ?>"><?php _e('Check Status', 'my-flying-box'); ?></button>
			<?php
